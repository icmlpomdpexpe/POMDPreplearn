...
class Discriminator(nn.Module):
    def __init__(self, z_dim, hidden_dim, total_rounds):
        super().__init__()
    ...
    def get_one(self, z, t):
        return self.networks[t](z)

    def get_till(self, z, t):
        return torch.cat([self.networks[i](z) for i in range(t+1)],-1)

    def create_network(self):
        net = nn.Sequential(
            nn.Linear(self.z_dim, self.hidden_dim), nn.Tanh(),
...
class BaseLearner(object):
    def __init__(
        ...
        self.phi_tilde = Feature(z_dim, action_dim, device, tau=tau, softmax=softmax).to(device)

        self.optimizer = optimizer

        self.reuse_weights = reuse_weights

        self.discriminators = Discriminator(z_dim, hidden_dim, num_update).to(device)
    ...
    def feature_learning(self, replay_buffer, T):
        ...
        for i in range(self.num_feature_update):
            z, actions, rewards, next_z = replay_buffer.sample(batch_size=self.batch_size)

            with torch.no_grad():
                dis_out = self.discriminators.get_till(next_z, T)
                feature = self.phi(z,actions)
